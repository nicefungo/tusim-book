...
"notes/chapter-21-exact-review-reseal-dispositions.md",
"notes/chapter-21-final-rereview-v8-dispositions.md",
...
def check_governance(run):
 ...
 exact_review=safe_regular(run,"inputs/notes/chapter-21-exact-review-reseal-dispositions.md").read_text()
 final_review=safe_regular(run,"inputs/notes/chapter-21-final-rereview-v8-dispositions.md").read_text()
 ...
 need("Initial verdicts: technical **REVISE**" in exact_review and "postreview-v7 seal before re-review" in exact_review and "This file records the immutable first exact-review findings" in exact_review,"exact-review-reseal-disposition")
 need("Review batch: `deleg_53e8dfc5`" in final_review and "all findings accepted for correction in a new postreview-v8 evidence run" in final_review,"final-rereview-v8-disposition")
 marker="corrected v8 reseal pending";need(marker in ledger and marker in report,"authorization-convergence")
def check_body(run,rid):
 ...
 for g in ["CH21_SOURCE_AUDIT PASS pin="+PIN,"CH21_BOUNDARY_AUDIT PASS pin="+PIN,"RELATION_MUTATION count_preserved=22 rejected=1","REPORT_ROLE_MUTATION rejected=1","BOUNDARY_MUTATION rejected=1","FORMULA_MUTATION stale_axis_rejected=1 linked_plugin_rejected=1","STATUS_MUTATION upstream_zero_mismatch_rejected=1","BOOK_INPUTS unchanged=1 head="]:
 ...
 inv=json.loads(safe_regular(run,"inventory.json").read_text());need(len(inv["source_target_pairs"])==23 and sum(r["make_target"] is not None for r in inv["source_target_pairs"])==22 and len(inv["reports"])==46,"inventory-size")
 ...
 cases={c["id"]:c for c in dec["worked_cases"]};need(set(cases)=={"dataflow-route-vs-formula","rounding-seed","retention-estimator","aspect-ratio-formula"},"decision-case-set")
 need(cases["dataflow-route-vs-formula"]["primary_matrix_independent_variables"]==["requested dataflow label","K"],"decision-dataflow-primary-axis")
 need(cases["dataflow-route-vs-formula"]["primary_matrix_controlled_variables"]==["M","N","PE rows=16","PE columns=16","bus width","pipeline depth"],"decision-dataflow-controls")
 need(cases["dataflow-route-vs-formula"]["secondary_sensitivity_variables"]==["report-local PE rows and columns varied together across 8, 16, and 32"],"decision-dataflow-sensitivity")
 need(cases["aspect-ratio-formula"]["primary_matrix_independent_variables"]==["M","N"] and cases["aspect-ratio-formula"]["primary_matrix_controlled_variables"]==["K=128","16x16 PE","pipeline depth 2","32 B/cycle"],"decision-aspect-primary")
 need(cases["aspect-ratio-formula"]["secondary_sensitivity_variables"]==["pipeline depth varied across 1, 2, and 4","bus width varied across 16, 32, and 64 B/cycle"],"decision-aspect-sensitivity")
 ...
 mr={r["id"]:r for r in met["rows"]};need(len(mr)==len(met["rows"]),"metric-ids-unique")
 need(mr["dataflow-sweep-local-cycles"]["formula"]=="WS=sum(pd*nc + K + pd*mc) + DMA_FP32_O; OS=sum(K) + DMA_FP32_O; RS=sum((pd-1)*nc + 1 + K + (pd-1)*mc) + DMA_FP32_O","metric-sweep-formula")
 need(mr["dataflow-report-local-cycles"]["formula"]=="WS=pd*tn + tm*tn*K + pd*tm + DMA_FP16_O; OS=tm*tn*K + DMA_FP16_O","metric-report-formula")
 need(mr["dataflow-linked-plugin-cycles"]["producer"]=="dataflow dispatcher plus WS/OS/RS plugin cycle methods" and mr["dataflow-linked-plugin-cycles"]["formula"]=="dispatcher sums per-tile plugin get_fill_cycles, execute_tile returned cycles, and get_drain_cycles" and "dataflow_dispatcher.c" in mr["dataflow-linked-plugin-cycles"]["source_path"],"metric-linked-producer")
