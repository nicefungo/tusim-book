...
15. **SRAM `total_size` omission discovered during field census — NEW FINDING, RESOLVED IN CLAIMS.** Save does not populate retained region `total_size`, while restore assigns it from the saved object. The source gate now encodes this gap and C18.22 is qualified; pinned source remains read-only.
