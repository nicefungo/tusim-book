...
The source audit also records a structural gap: SRAM save copies explicit aggregate bank fields and region names, but not `total_size`; restore assigns `total_size` from the zero-initialized saved region. This is not evidence of a transparent whole-object snapshot.
