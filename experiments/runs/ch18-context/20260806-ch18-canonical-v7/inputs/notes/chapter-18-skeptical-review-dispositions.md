...
16. **`TU_CTX_COMPLETED` called schedulable — POST-v6 REVIEW CORRECTION, RESOLVED.** Exact-commit manuscript review found that C18.11 used “schedulable” for a printable enum value even though both selectors admit READY descriptors only and no implementation path produces COMPLETED. The live ledger and manuscript now say printable lifecycle vocabulary, explicitly not scheduler-eligible. Canonical v6 remains immutable but is superseded for final drafting authority by the post-correction seal.
