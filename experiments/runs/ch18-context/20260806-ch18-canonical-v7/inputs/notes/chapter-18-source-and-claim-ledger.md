...
- **C18.11 (qualified):** READY, BLOCKED, COMPLETED, and IDLE descriptors can all be freed despite header prose saying IDLE or COMPLETED. `TU_CTX_COMPLETED` is printable lifecycle vocabulary, but scheduling selects READY descriptors only and no pinned implementation path produces COMPLETED.
...
- **C18.22 (qualified):** Save/restore explicitly copies region `total_size`, bank capacity/geometry, aggregate access/conflict/stall counters, bandwidth-policy scalars, region cycle, and name, but deliberately drops saved `bw_banks`. Region and aggregate metadata can therefore be restored from one context while live per-bank arbitration state remains from another.
