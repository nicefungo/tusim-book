...
**Predraft status:** open; drafting remains blocked until skeptical review and a final post-review canonical seal pass.
...
- **C18.5 (qualified):** `active_count` counts allocated descriptors, not ACTIVE owners. Correct ownership requires the tuple `(active_ctx_id, every descriptor state, active_count)`; the implementation does not enforce exactly one ACTIVE descriptor.
...
- **C18.15 (verified):** Slice expiry uses inclusive thresholds (`>=`) independently for caller-fed cycles and commands. Zero disables that threshold. The manager does not trigger a switch automatically.
- **C18.16 (rejected):** Notification accounting is not per-context execution accounting. `tu_ctx_notify_command()` and `tu_ctx_notify_cycles()` update only manager slice counters, even when no descriptor is ACTIVE. `total_commands` has no producer; `total_cycles` changes only on save from a difference of legacy core `estimated_cycles` and `last_switch_cycle`.
- **C18.17 (qualified):** `total_cycles_stolen` is a manager-local analytical ledger. Restore adds fixed cost plus `ceil((pending outgoing bytes + incoming saved bytes)/state_bytes_per_cycle)` when bandwidth is nonzero. It does not advance legacy core time, command-queue time, global DMA time, SRAM-bank time, or Chapter 17 producers.
...
- **C18.22 (verified):** Save/restore copies aggregate SRAM metadata—capacity, bank geometry, aggregate access/conflict/stall counters, bandwidth-policy scalars, and region cycle—but deliberately drops saved `bw_banks`. Restore preserves the live core's existing per-bank meter pointer and contents. Aggregate metadata and arbitration state can therefore come from different contexts.
- **C18.23 (rejected):** Double-buffer ownership is not context-retained. Saved `db` pointers are nulled and restore sets every live region `db=NULL`; Chapter 16 remains authoritative for the resulting ownership boundary.

## Queue, DMA, synchronization, plugin, and precision state

- **C18.24 (rejected):** Header/docs claims of command-queue snapshot/restore are false. Save stores only the queue pointer; restore deliberately keeps the live pointer and does not restore commands, dependencies, signals, indices, counts, or queue clock. The probe preserves discriminating live queue sentinels across a context switch.
- **C18.25 (qualified):** `tu_core_sync()` swaps the selected core state into `g_tu`, calls only `tu_cmdq_sync_all()`, and swaps back. It drains that command queue only; it does not flush descriptor DMA or prove every copied/dropped subsystem quiescent.
...
- **C18.29 (rejected):** Precision/rounding-state snapshot claims are false for the process-global rounding mode and xorshift PRNG state. Both continue globally across context switches; neither is in `tu_state_t` or the context manager.
...
- **C18.32 (rejected):** `save_dram_state` is declared but never copied into the manager or consumed. `has_config_override`, `config_override`, and `user_data` remain descriptor metadata with no switching/scheduling effect.
...
- **C18.34 (verified):** Injected first/second/third SRAM-copy allocation failures during initial context allocation return `-1`, leave the slot IDLE, leave `active_count=0`, and free every partial retained allocation.
...
Drafting remains prohibited until the 32-hash/147-predicate source audit, focused mutation, exhaustive transition probe, sweep recomputation, static-link checks, immutable manifests, optimization-safe validator, independent skeptical review, and final post-review seal all pass. Any review-driven change to bundled inputs requires a new retained seal; older runs remain immutable history.
