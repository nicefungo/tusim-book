...
RUN_ID=${CH18_RUN_ID:-20260805-ch18-canonical-v1}
...
body(){
 ...
 grep -F "CH18_SOURCE_AUDIT PASS pin=$PIN hashes=32 predicates=147 checks=179" "$RUN_DIR/source-audit.log"
 set +e
 ...
 grep -Fx "tu_cmodel/infra/tu_context.o" "$RUN_DIR/archive-members.log"
...
PY
 ...
 for b in test-context test-context-mut test-context-sweep ch18-probe ch18-probe-o2; do
   readelf -d "$WORK/$b" > "$RUN_DIR/$b-readelf.log"
   ! grep -Eq 'NEEDED.*libtucmodel' "$RUN_DIR/$b-readelf.log"
 done
 echo "STATIC_LINK PASS binaries=5"
 "$WORK/test-context" > "$RUN_DIR/test-context.log" 2>&1
 grep -F "15/15 tests passed" "$RUN_DIR/test-context.log"
 echo "FOCUSED_CONTEXT PASS tests=15"
 set +e
 "$WORK/test-context-mut" > "$RUN_DIR/test-context-mutation.log" 2>&1
 mut_rc=$?
 set -e
 [[ $mut_rc -ne 0 ]]
 grep -F "14/15 tests passed" "$RUN_DIR/test-context-mutation.log"
 echo "FOCUSED_MUTATION PASS tests=14/15 rc=$mut_rc"
 "$WORK/test-context-sweep" > "$RUN_DIR/test-context-sweep.log" 2>&1
 ...
 "$WORK/ch18-probe" > "$RUN_DIR/probe.log" 2>/dev/null
 "$WORK/ch18-probe-o2" > "$RUN_DIR/probe-o2.log" 2>/dev/null
 ...
 echo "PROBE PASS failures=0 optimization_stable=yes"
