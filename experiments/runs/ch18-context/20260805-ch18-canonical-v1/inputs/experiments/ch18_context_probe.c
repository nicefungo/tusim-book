...
void *__wrap_malloc(size_t n) {
    if (malloc_armed && ++malloc_seen == malloc_fail_at) return NULL;
...
static void print_vec(const char *row, const char *phase, const env_t *e, int rc) {
    ...
    printf(" gdma=%u,%d,%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%u round=%d\n",
...
static void test_config_and_allocation(void) {
    ...
    CHECK(valid == 0 && zero != 0 && policy != 0 && scope != 0 && live != 0,
          "config validation");
    tu_dma_destroy(); tu_core_destroy(core);
...
static void test_switch_ownership(void) {
    ...
    e=env_make(&c); tu_ctx_alloc(e.mgr); tu_ctx_alloc(e.mgr);
    print_vec("restore_direct", "PRE", &e, 0); rc=tu_ctx_restore(e.mgr,1); print_vec("restore_direct", "POST", &e, rc);
    CHECK(rc==0 && active_descriptors(e.mgr)==2 && e.mgr->active_ctx_id==1, "direct restore creates two active"); env_drop(&e);
...
static void test_scheduling_and_notifications(void) {
    ...
    printf("ROW slice_thresholds RESULT initial=%d c9=%d c10=%d reset=%d m2=%d m3=%d\n",
           tu_ctx_slice_expired(e.mgr),
           (tu_ctx_notify_cycles(e.mgr,9),tu_ctx_slice_expired(e.mgr)),
           (tu_ctx_notify_cycles(e.mgr,1),tu_ctx_slice_expired(e.mgr)),
           (e.mgr->slice_cycles_used=0,e.mgr->slice_cmds_used=0,tu_ctx_slice_expired(e.mgr)),
           (tu_ctx_notify_command(e.mgr),tu_ctx_notify_command(e.mgr),tu_ctx_slice_expired(e.mgr)),
           (tu_ctx_notify_command(e.mgr),tu_ctx_slice_expired(e.mgr)));
    CHECK(tu_ctx_slice_expired(e.mgr),"slice exact threshold");
    ...
    CHECK(e.mgr->slice_cmds_used==4 && e.mgr->slice_cycles_used==5 && tc==0 && e.mgr->contexts[0].total_commands==0,
...
static void test_malloc_failures(void) {
    ...
    for (int k=1;k<=3;k++) {
        ...
        CHECK(rc==-1 && e.mgr->active_count==0 && e.mgr->contexts[0].state==TU_CTX_IDLE &&
              ...
              "initial allocation failure cleanup"); env_drop(&e);
    }
    ...
    CHECK(rc==-1 && e.mgr->contexts[0].state==TU_CTX_ACTIVE &&
          e.mgr->contexts[0].hw_state.sram_w.banks.data==NULL &&
          e.mgr->contexts[0].hw_state.sram_a.banks.data==NULL,
          "resave failure destroys prior snapshot non-atomically"); env_drop(&e);
}
...
static void test_adjacent_state(void) {
    ...
    printf("ROW rounding_prng_global RESULT a=%.17g b=%.17g ref=%.17g/%.17g mode=%d\n",a,b,ra,rb,(int)tu_get_rounding_mode());
    CHECK(a==ra && b==rb && tu_get_rounding_mode()==TU_ROUND_RTZ,"rounding mode and PRNG global"); env_drop(&e);
