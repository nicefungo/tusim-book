...
static void test_scheduler_analysis_and_validation(void) {
    ...
    tu_instruction_t reverse_seq[] = {
        I(TU_ISA_RELU, 0, 4, 2, 0, 0x22222222),
        I(TU_ISA_DMA_STORE, 0, 16, 1, 2, 0x11111111),
    };
    CHECK(tu_sched_build_dag(&g, reverse_seq, 2, &cfg) == 0,
          "reverse barrier-report DAG");
    int reverse = tu_sched_insert_barriers(&g);
    printf("SCHED_BARRIER_DIRECTION store_then_compute=%d compute_then_store=%d\n",
           direct, reverse);
    CHECK(direct == 1 && reverse == 0,
          "barrier reporting is asymmetric and count only");
...
static void test_cross_pass_strided_and_opcode_census(void) {
    tu_instruction_t x = I(TU_ISA_DMA_LOAD_STRIDED, 8, 16, 0, 4, 0);
    tu_sram_access_t acc;
    tu_sched_analyze_access(&x, &acc);
    tu_liveness_result_t r;
    CHECK(tu_live_analyze(&x, 1, &r) == 0, "strided DMA liveness");
    printf("CROSS_STRIDED sched_writes=%d/%d/%d live_vregs=%u",
           acc.writes[TU_SRAM_W], acc.writes[TU_SRAM_A], acc.writes[TU_SRAM_O],
           r.num_vregs);
    ...
    putchar('\n');
    CHECK(acc.writes[TU_SRAM_A] && !acc.writes[TU_SRAM_W],
          "scheduler strided channel uses flags bits three to two");
    CHECK(r.num_vregs == 2 && r.vregs[0].region == TU_VREG_W &&
          r.vregs[1].region == TU_VREG_A && r.vregs[1].first_def == -1,
          "liveness strided path uses different channel bits and broad use class");

    unsigned rows = 0;
    for (unsigned op = 0; op < (unsigned)TU_ISA_OPCODE_COUNT; ++op) {
        tu_instruction_t c = I((tu_isa_opcode_t)op, 8, 4, 2, 0, 0x00100020);
        tu_sram_access_t sa;
        tu_liveness_result_t lr;
        tu_sched_analyze_access(&c, &sa);
        CHECK(tu_live_analyze(&c, 1, &lr) == 0, "numeric opcode census analysis");
        unsigned mask = 0;
        for (unsigned reg = 0; reg < TU_SRAM_REGION_COUNT; ++reg) {
            if (sa.reads[reg]) mask |= 1u << reg;
            if (sa.writes[reg]) mask |= 1u << (reg + 3);
        }
        printf("OPCODE_CENSUS op=0x%02x sched_mask=0x%02x live_vregs=%u",
               op, mask, lr.num_vregs);
        for (uint32_t i = 0; i < lr.num_vregs; ++i)
            printf("/%d:%d:%d:%u", lr.vregs[i].region,
                   lr.vregs[i].first_def, lr.vregs[i].last_use,
                   lr.vregs[i].size_bytes);
        putchar('\n');
        rows++;
    }
    printf("OPCODE_CENSUS_SUMMARY rows=%u\n", rows);
    CHECK(rows == (unsigned)TU_ISA_OPCODE_COUNT && rows == 128,
          "complete numeric opcode census");
}
...
static void test_liveness_public_api_boundaries(void) {
    tu_instruction_t defs[] = {
        I(TU_ISA_DMA_LOAD, 0, 16, 0, 0, 0),
        I(TU_ISA_DMA_LOAD, 8, 16, 0, 0, 0),
    };
    tu_liveness_result_t r;
    CHECK(tu_live_analyze(defs, 2, &r) == 0, "repeat-build analysis");
    tu_live_build_interference(&r);
    uint32_t first_nodes = r.graph_w.num_vregs;
    bool *first_matrix = r.graph_w.interference;
    tu_live_build_interference(&r);
    printf("LIVE_REBUILD nodes=%u->%u matrix_replaced=%d edges=%u\n",
           first_nodes, r.graph_w.num_vregs,
           first_matrix != r.graph_w.interference, edge_count(&r.graph_w));
    CHECK(first_nodes == 2 && r.graph_w.num_vregs == 4 &&
          first_matrix != r.graph_w.interference,
          "repeated interference construction appends and replaces matrix");
    free(first_matrix);
    free_graphs(&r);

    tu_instruction_t source = I(TU_ISA_DMA_LOAD, 0, 16, 0, 0, 0);
    tu_instruction_t unrelated = I(TU_ISA_RELU, 77, 3, 0, 0, 0);
    CHECK(tu_live_analyze(&source, 1, &r) == 0, "provenance source analysis");
    r.vregs[0].physical_offset = 32;
    tu_allocated_sequence_t out;
    tu_live_config_t cfg = tu_live_config_default;
    int prc = tu_live_apply(&r, &unrelated, 1, &cfg, &out);
    printf("LIVE_PROVENANCE rc=%d valid=%d opcode=%s dim0=%u\n",
           prc, out.valid, tu_isa_opcode_name((tu_isa_opcode_t)out.instructions[0].opcode),
           out.instructions[0].dim0);
    CHECK(prc == 0 && out.valid && out.instructions[0].opcode == TU_ISA_RELU &&
          out.instructions[0].dim0 == 77,
          "low-level apply accepts unrelated analysis result");

    memset(&r, 0, sizeof(r));
    r.num_vregs = 1;
    r.vregs[0].id = 1; r.vregs[0].region = TU_VREG_W;
    r.vregs[0].size_bytes = 16; r.vregs[0].first_def = 0;
    r.vregs[0].last_use = 1; r.vregs[0].physical_offset = UINT32_MAX;
    r.graph_w.vregs[0] = &r.vregs[0]; r.graph_w.num_vregs = 1;
    r.graph_w.interference = calloc(1, sizeof(bool));
    CHECK(r.graph_w.interference != NULL, "spill-accounting graph allocation");
    cfg = tu_live_config_default;
    cfg.w_capacity = 8; cfg.safety_margin = 0; cfg.enable_spilling = true;
    cfg.spill_strategy = (tu_spill_strategy_t)99;
    tu_live_color(&r, &cfg);
    printf("LIVE_SPILL_ACCOUNTING spilled=%d num_spills=%u spill_bytes=%u colored=%d offset=%u\n",
           r.vregs[0].spilled, r.num_spills, r.spill_bytes,
           r.graph_w.colored, r.vregs[0].physical_offset);
    CHECK(r.vregs[0].spilled && r.num_spills == 2 && r.spill_bytes == 32 &&
          !r.graph_w.colored && r.vregs[0].physical_offset == UINT32_MAX,
          "one unplaced value is counted twice");
    free_graphs(&r);
    ...
    r.vregs[0].id = 1; r.vregs[0].region = TU_VREG_W;
    r.vregs[0].size_bytes = 16; r.vregs[0].first_def = 0;
    r.vregs[0].last_use = 0; r.vregs[0].physical_offset = UINT32_MAX;
    r.graph_w.vregs[0] = &r.vregs[0]; r.graph_w.num_vregs = 1;
    r.graph_w.interference = calloc(1, sizeof(bool));
    CHECK(r.graph_w.interference != NULL, "invalid-allocation graph allocation");
    cfg = tu_live_config_default;
    cfg.w_capacity = 32; cfg.safety_margin = 0; cfg.enable_spilling = false;
    cfg.alloc_strategy = (tu_alloc_strategy_t)99;
    tu_live_color(&r, &cfg);
    printf("LIVE_INVALID_ENUM alloc=99 offset=%u colored=%d\n",
           r.vregs[0].physical_offset, r.graph_w.colored);
    CHECK(r.vregs[0].physical_offset == 0 && r.graph_w.colored,
          "invalid allocation enum is accepted");
    free_graphs(&r);
}
