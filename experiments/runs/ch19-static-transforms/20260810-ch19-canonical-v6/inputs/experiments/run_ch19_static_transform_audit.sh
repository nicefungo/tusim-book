...
PY
cc -std=c11 -O0 -I"$ARCHIVE" -I"$ARCHIVE/tu_cmodel" -I"$ARCHIVE/tu_cmodel/isa" -o "$TMP/probe-sched-mut" \
...
p.write_text(s.replace(old,new))
PY
cc -std=c11 -O0 -I"$ARCHIVE" -I"$ARCHIVE/tu_cmodel" -I"$ARCHIVE/tu_cmodel/isa" -o "$TMP/probe-live-mut" \
...
grep -q 'CHECK_FAIL two W, repeated implicit A, O definitions' "$RUN_DIR/logs/12-mutation-liveness-opcode.log"
...
# Chapter 19 canonical predraft audit — $RUN_ID
...
(
  ...
  (cd inputs && sha256sum -c ../INPUT_SHA256SUMS >/dev/null)
)
