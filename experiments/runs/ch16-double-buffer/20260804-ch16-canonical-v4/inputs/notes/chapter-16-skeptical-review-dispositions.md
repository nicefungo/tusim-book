...
11. **R11 — Bounds arithmetic: NEW FINDING.** General SRAM and descriptor checks form `addr + size` or `base + total_bytes` in 32-bit arithmetic before comparison. The bounded canonical shapes are safe; the API does not establish overflow-safe general validation.

12. **R12 — Live reinitialization: NEW FINDING.** `tu_sram_init_bw()` starts with `memset` and allocates new primary and bank-meter storage without releasing live primary, meter, shadow, or DB-state allocations. Destroy-before-init is mandatory. The amended probe captures the pointer loss and explicitly salvages the old pointers.

13. **R13 — Shared-resource overlap equation: NEW FINDING.** Independent `min(C,P)+min(C,S)` credits need proven independent resources. A discriminating `C=100,P=80,S=80` case gives uncapped 160 versus a shared-window cap of 100. The sampled historical rows happen not to over-credit, but the equation is not generally valid.

14. **R14 — Runner fail-fast integrity: NEW FINDING.** The earlier runner invoked `body` while `set +e` was active, so a stale grep could fail without stopping the body. Canonical v3 therefore remains useful retained evidence but is not final drafting authority. The amended runner uses a pipe with `pipefail` and captures `PIPESTATUS[0]`; canonical v4 must prove every gate actually controls exit status.
...
**Post-amendment gate:** REOPENED. Canonical v1 is provisional, v2 is validator-blocked, and v3 is superseded after the complete skeptical-review result exposed R11–R14 and the fail-fast runner defect. Drafting remains blocked pending a clean canonical v4 containing the full lifecycle, bounds, resource-cap, ASan, and fail-fast controls.
