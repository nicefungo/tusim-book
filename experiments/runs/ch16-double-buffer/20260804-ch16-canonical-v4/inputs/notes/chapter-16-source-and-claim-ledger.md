...
**Predraft status:** reopened; canonical v3 is superseded and canonical v4 is required before drafting.
...
- **C16.29 (verified):** post-review canonical v3 retains and gates the exact standalone, invalid-bridge, depth-1 no-load, depth-2 ledger, reset, and clock observations with `failures=0`.
- **C16.30 (verified):** v3 passes 31 source hashes, 51 predicates/82 checks, hash mutation, archive/static-link gates, direct 10/10 focused execution, 9/10 assertion mutation, and source/book provenance.
- **C16.31 (verified/qualified):** v3 recomputes all ten historical table rows and the 17/18.285714/20 threshold contradiction; the values remain analytical and uncalibrated.
- **C16.32 (verified):** the skeptical review independently rebuilt and hand-recomputed the v1 evidence, its findings were incorporated, and post-review v3 passed. Drafting is authorized only from v3 plus the post-seal gate-closure record.
- **C16.33 (rejected):** `tu_sram_init()` is not reset or safe reinitialization. Reinitializing a live region overwrites primary, bank-meter, shadow, and DB-state ownership without freeing the old allocations. Destroy-before-init is required.
- **C16.34 (rejected):** context restore is incompatible with live double-buffer ownership. It restores primary bytes and nulls the live `db` pointer without freeing shadow/state, discarding role, shadow bytes, dirty state, and ledgers.
- **C16.35 (qualified):** bounded canonical shapes are safe, but general SRAM and descriptor range checks use wrap-prone 32-bit addition; no overflow-safe API-wide guarantee is established.
- **C16.36 (qualified):** separate preload/store overlap credits require proven independent resources. With a shared resource, combined credit must be capped to one compute window.
- **C16.37 (blocked):** C16.29–C16.32 describe the superseded v3 state. Drafting remains blocked until canonical v4 bundles and gates C16.33–C16.36 plus fail-fast runner behavior.
