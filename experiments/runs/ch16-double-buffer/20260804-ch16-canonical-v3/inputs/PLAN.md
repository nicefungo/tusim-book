...
Primary evidence: `infra/tu_context.[ch]`, `isa/tu_scheduler.[ch]`, `isa/tu_liveness.[ch]`, focused tests, context and scheduler sweeps, and docs. It closes the remaining context/liveness coverage without reviving the broken ONNX-to-runtime narrative. Its framing has a mandatory split/keep gate because runtime context retention, compile-time schedule ordering, and liveness allocation may not support one coherent reader decision.

### Chapter 19 — Verification as an Architectural Feature

Primary evidence: the repository's unit, invariant, golden, differential, random, configuration, mutation, integration, regression/CI, replay/debug, DPI, and binding surfaces plus the sealed evidence practices demonstrated in Chapters 1–18. Keep this chapter about choosing evidence for a claim; do not repeat Chapter 17's metric-producer taxonomy or turn prior chapter audit logs into the chapter structure.

### Chapter 20 — Designing a Trustworthy Sweep
...
### Chapter 21 — Lessons from the Exploration Portfolio

Primary evidence: the exploration report portfolio reconciled against executable source and the chapter audits. Keep this chapter empirical and synthetic: recurring regimes, useful alternatives, stale conclusions, and cross-domain lessons. Do not repeat Chapter 20's sweep-construction procedure.

### Chapter 22 — Extending Tusim Without Breaking Its Contract
...
| **Chapter 17/19 overlap** | Metric provenance and verification evidence can collapse into a repetitive “measurement and testing” discussion. | Chapter 17 or 19 framing includes the other's primary reader decision. | Keep Chapter 17 on producer/interval/units/fidelity; keep Chapter 19 on evidence selection, failure interpretation, and verification architecture. Re-scope before drafting if that distinction cannot be maintained. |
| **Chapter 18 overload** | Context save/restore, scheduler ordering, and liveness allocation have different timescales, consumers, and integration status. | Fresh reconnaissance cannot express one decision matrix without combining heterogeneous clocks or inventing a pipeline. | Mandatory split/keep review during framing. Split into separate chapters if one coherent decision and evidence ladder cannot cover all three honestly; update chapter count and downstream numbering only through a governed replan. |
| **Chapter 20/21 repetition** | Sweep methodology and portfolio synthesis draw on the same reports and can duplicate examples. | The same section could appear unchanged in both chapters. | Chapter 20 owns how to design and validate a sweep; Chapter 21 owns what the existing portfolio teaches after executable reconciliation. Use cross-references rather than duplicate tutorials. |
| **Broad synthesis chapters becoming catalogues** | Chapters 19–22 cover many modules and may regress to API/report enumeration. | A framing plan lacks a single reader decision, ranked alternatives, or explicit exclusions. | Split or narrow before evidence sealing; organize by decisions and failure modes, not directories or document lists. |
| **Completed-chapter supplement backlog is forgotten** | Chapters 8, 10, and 14 have bounded reader-decision gaps that do not justify new chapters but still matter to first-edition completeness. | End of Chapter 19 or start of publication preparation. | Schedule and verify: Chapter 8 rounding experiment/replay contract; Chapter 10 address-generation status/safe subset; Chapter 14 concise fusion, convolution/pooling, and softmax/normalization architecture-decision material. Keep deeper attention claims deferred until correctness is repaired. |
...
2. **After Chapter 16 / Part IV boundary:** run a full whole-book coverage and sequence review. Confirm that memory supply, movement, buffering, operators, and off-chip service are neither missing a reader decision nor double-covered.
3. **During Chapter 18 framing:** make and record the mandatory split/keep decision before any canonical audit or manuscript drafting.
4. **After Chapter 18 / Part V boundary:** verify the Chapter 17/18 division, Chapter 19 prerequisites, and whether any producer or resource-over-time model remains unnamed.
5. **After Chapter 19:** review Chapter 20/21 overlap and turn the completed-chapter supplement backlog into an explicit revision schedule.
...
## 9. Session rule after Chapter 15

Chapter 15 is complete locally. Chapter 16 — Double Buffering and Legal Overlap — is next and must begin with fresh framing against `PLAN.md`, the standalone `memory/double_buffer.[ch]` surface, Chapters 9/10/14 boundaries, and the pinned tests/docs. Do not merge those adjacent modules into a fictional pipeline. Chapter 15 publication remains approval-gated: request explicit user authorization before any rebuild-tree update or push to `origin/main`.
