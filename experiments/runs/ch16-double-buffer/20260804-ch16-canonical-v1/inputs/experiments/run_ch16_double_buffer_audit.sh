...
RUN_ID=${CH16_RUN_ID:-20260804-ch16-canonical-v1}
...
body() {
  ...
  grep -F "CH16_SOURCE_AUDIT PASS pin=$PIN hashes=25 predicates=42 checks=67" "$RUN_DIR/source-audit.log"
...
(
  ...
  sha256sum input-hashes.txt input_commit source_pin tusim-ignored-before.sha256 tusim-ignored-after.sha256 \
    ...
    test-double-readelf.log probe-readelf.log test-double.log test-double-mutation.log probe.log report-recompute.log transcript.log
