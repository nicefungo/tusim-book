...
- **C16.5 (qualified):** `shadow_dirty` is caller notification state only. `tu_sram_get_shadow_ptr()` does not mark dirty despite the header claim; only `tu_sram_notify_shadow_write()` does. Notification does not write bytes, validate `bytes <= buffer_size`, bind a descriptor, or prove completion.
...
- **C16.22 (qualified):** context save explicitly omits double-buffer state and restore sets each live region's `db=NULL`. It preserves primary bytes according to save scope, not active/shadow ownership, shadow bytes, dirty state, swaps, or ledgers. Existing live `db` ownership must be audited for leakage/loss before any stronger claim.
