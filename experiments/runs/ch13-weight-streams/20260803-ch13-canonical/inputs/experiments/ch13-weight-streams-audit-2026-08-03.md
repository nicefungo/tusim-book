...
- **Canonical retained run:** `experiments/runs/ch13-weight-streams/<date>-ch13-canonical/`
...
The runner may be re-executed with a new `CH13_RUN_ID`; earlier runs are immutable historical evidence. The first sealed attempt is canonical; any later reseal supersedes it only through the same fail-closed gates.
