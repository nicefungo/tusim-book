...
**Status:** scope selected from whole-book replanning and fresh source reconnaissance; drafting blocked pending ledger, canonical audit, skeptical review, and predraft closure.
...
| 2 | Measurement Surfaces | choose producer/interval/units across counters, trace, cycle, and power modules | requires every major metric producer to be named first; DRAM is still missing | next Chapter 16 |
| 3 | Resource Multiplexing | compare contexts, scheduling, and liveness | important remaining gap but independent of the off-chip feeding arc | planned Chapter 17 |
| 4 | Verification as Architecture | choose a verification rung and fail-closed gate | would synthesize before all major source surfaces are taught | planned Chapter 18 |
| 5 | Trustworthy Sweeps | design controlled explorations | DRAM's historical sweep is itself an important counterexample that should be audited first | planned Chapter 19 |
...
- full performance-counter, trace, cycle-model, and power-model contracts (Chapter 16);
