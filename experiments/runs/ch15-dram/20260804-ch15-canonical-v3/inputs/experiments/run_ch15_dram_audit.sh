...
RUN_ID="${CH15_RUN_ID:-20260804-ch15-canonical-v3}"
...
INPUTS=(
  ...
  experiments/ch15_sweep_recompute.py
...

{
  ...
  grep -q "CH15_SOURCE_AUDIT PASS pin=$PIN hashes=23 predicates=62 checks=85" "$RUN_DIR/source-audit.log"
  ...
  cc -O2 -Wall -Wextra -std=c11 -I"$WORK" -I"$WORK/tu_cmodel" \
    -o "$WORK/ch15-test-memhier" "$WORK/tests/test_memory_hierarchy.c" "$WORK/libtucmodel.a" -lm
  cc -O2 -Wall -Wextra -std=c11 -I"$WORK" -I"$WORK/tu_cmodel" \
    -o "$WORK/ch15-test-cycle" "$WORK/tests/test_cycle_model.c" \
    "$WORK/tu_cmodel/perf/cycle_model.c" "$WORK/libtucmodel.a" -lm
  cc -O2 -Wall -Wextra -std=c11 -I"$WORK" -I"$WORK/tu_cmodel" \
    -o "$WORK/ch15-test-power" "$WORK/tests/test_power_model.c" "$WORK/libtucmodel.a" -lm
  cc -O2 -Wall -Wextra -std=c11 -I"$WORK" -I"$WORK/tu_cmodel" \
    -o "$WORK/ch15-probe" "$RUN_DIR/inputs/experiments/ch15_dram_probe.c" "$WORK/libtucmodel.a" -lm
  for exe in ch15-test-dram ch15-test-memhier ch15-test-cycle ch15-test-power ch15-probe; do
  ...
  grep -q "exited normally" "$RUN_DIR/test-dram.log"
  echo "FOCUSED_TEST PASS 12/12"

  run_gdb "$WORK/ch15-test-memhier" "$RUN_DIR/test-memory-hierarchy.log"
  grep -q "10/10 tests passed" "$RUN_DIR/test-memory-hierarchy.log"
  grep -q "exited normally" "$RUN_DIR/test-memory-hierarchy.log"
  echo "MEMHIER_TEST PASS 10/10"

  run_gdb "$WORK/ch15-test-cycle" "$RUN_DIR/test-cycle-model.log"
  grep -q "Tests: 21 run, 21 passed, 0 failed" "$RUN_DIR/test-cycle-model.log"
  grep -q "exited normally" "$RUN_DIR/test-cycle-model.log"
  echo "CYCLE_MODEL_TEST PASS 21/21 source-linked-not-archive-member=yes"

  run_gdb "$WORK/ch15-test-power" "$RUN_DIR/test-power-model.log"
  grep -q "Results: 20/20 tests passed" "$RUN_DIR/test-power-model.log"
  grep -q "exited normally" "$RUN_DIR/test-power-model.log"
  echo "POWER_MODEL_TEST PASS 20/20"
...
PY
  ...
  grep -q "exited with code 01" "$RUN_DIR/test-dram-mutation.log"
  ...
  grep -q "exited normally" "$RUN_DIR/ch15-probe.log"
  echo "PROBE PASS"

  python3 "$RUN_DIR/inputs/experiments/ch15_sweep_recompute.py" \
    "$WORK/docs/exploration/dram-type-clock-sweep.md" | tee "$RUN_DIR/sweep-recompute.log"
  grep -q "SWEEP_RECOMPUTE PASS contradictions=4 historical_report_not_decision_evidence=yes" "$RUN_DIR/sweep-recompute.log"
  echo "SWEEP_AUDIT PASS"
...
(
  ...
  printf '%s\n' source-archive-sha256.txt source-audit.log source-audit-mutation.log \
    archive-members.txt input-hashes.txt build.log test-dram.log test-memory-hierarchy.log \
    test-cycle-model.log test-power-model.log test-dram-mutation.log ch15-probe.log \
