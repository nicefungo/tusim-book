...
int main(void) {
    ...
    tu_dram_read(h2, 0, 64, &cyc, &st);
    printf("ACCESS overwrite_same_channel cycles=%llu stall=%llu ch0_avail=%llu\n",
           (unsigned long long)cyc, (unsigned long long)st,
           (unsigned long long)h2->channel_available_cycle[0]);
    CHECK(cyc == 50 && st == 1050 && h2->channel_available_cycle[0] == 50,
          "channel availability overwrite");
    ...
    tu_dram_model_t *meter = tu_dram_create(TU_DRAM_TYPE_HBM2);
    tu_dram_read(meter, 0, 0, &cyc, &st); /* establish window start at cycle 0 */
    for (int i = 0; i < 1000; i++) tu_dram_tick(meter);
    unsigned first_bw_stall = 0;
    for (unsigned i = 1; i <= 2001; i++) {
        tu_dram_read(meter, (uint64_t)(i % 8) * 64, 64, &cyc, &st);
        if (st > 50 && first_bw_stall == 0) first_bw_stall = i;
    }
    printf("METER first_bw_stall_request=%u pending=%llu budget=%llu current=%llu\n",
           first_bw_stall, (unsigned long long)meter->pending_read_bytes,
           (unsigned long long)meter->bandwidth_available,
           (unsigned long long)meter->current_cycle);
    CHECK(first_bw_stall == 2001 && meter->pending_read_bytes == 128064 &&
          meter->bandwidth_available == 127936 && meter->current_cycle == 1000,
          "meter double accounting");

    tu_dram_model_t *resetm = tu_dram_create(TU_DRAM_TYPE_HBM2);
    tu_dram_read(resetm, 0, 0, &cyc, &st); /* establish window start at cycle 0 */
    for (int i = 0; i < 1001; i++) tu_dram_tick(resetm);
    tu_dram_reset(resetm);
    uint64_t stale_start = resetm->bw_window_start;
    uint64_t stale_size = resetm->bw_window_size_cycles;
    tu_dram_read(resetm, 0, 64, &cyc, &st);
    printf("RESET stale_start=%llu stale_size=%llu cycles=%llu stall=%llu budget=%llu current=%llu\n",
           (unsigned long long)stale_start, (unsigned long long)stale_size,
           (unsigned long long)cyc, (unsigned long long)st,
           (unsigned long long)resetm->bandwidth_available,
           (unsigned long long)resetm->current_cycle);
    CHECK(stale_start == 1000 && stale_size == 1000 && cyc == 50 && st == 0 &&
          resetm->bandwidth_available == 255936 && resetm->current_cycle == 0,
          "reset stale window");
    ...
    uint64_t hstall = 0;
    int hrc = tu_mem_hierarchy_read(&mh, TU_MEM_DRAM, NULL, 0, marker, sizeof(marker), &hstall);
    int marker_unchanged = 1;
    for (size_t i = 0; i < sizeof(marker); i++) {
        if (marker[i] != 0x5a) marker_unchanged = 0;
    }
    printf("HIER type=%d rc=%d stall=%llu marker=0x%02x unchanged64=%d dram_cycle=%llu\n",
           mh.dram ? (int)mh.dram->type : -1, hrc, (unsigned long long)hstall,
           marker[0], marker_unchanged,
           mh.dram ? (unsigned long long)mh.dram->current_cycle : 0ULL);
    CHECK(mh.dram && mh.dram->type == TU_DRAM_TYPE_HBM2 && hrc == 0 &&
          hstall == 1000 && marker_unchanged && mh.dram->current_cycle == 0,
          "hierarchy boundary");
    ...
    tu_dram_destroy(resetm);
    tu_dram_destroy(meter);
