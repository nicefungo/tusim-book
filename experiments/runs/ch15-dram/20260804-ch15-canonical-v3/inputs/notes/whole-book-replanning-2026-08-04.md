...
## Completed-chapter supplement backlog

The cross-chapter audit distinguishes reader-decision coverage from mention-level coverage. It does not justify renumbering or reopening every chapter now, but it records these bounded supplements for revision/publication preparation:

- **Chapter 8:** add a compact executable rounding-experiment and replay contract (error growth/statistical checks) without splitting BF16/FP8/TF32 into new chapters.
- **Chapter 10:** add a short address-generation status and safe-subset section; general transposed correctness remains deferred until repaired.
- **Chapter 14:** add concise architecture-decision material for fusion/activation, convolution/pooling mapping, and softmax/normalization algorithm choices. Do not claim deeper attention coverage while arbitrary-input correctness is broken.
- **Compiler:** keep the dedicated ONNX demonstration chapter deferred until repository-contained nontrivial TU lowering links, runs, and verifies output.

These are targeted supplements, not a return to the obsolete 29-chapter taxonomy.
