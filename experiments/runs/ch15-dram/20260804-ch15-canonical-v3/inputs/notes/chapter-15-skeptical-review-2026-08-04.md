...
**Interim disposition after amended seal:** canonical v2 passed at input commit `12267d02e7c155251f49269730269d3f76d88321`, but the completed asynchronous review later exposed residual audit-coverage and probe-object-size gaps. **Current disposition: DRAFT_ALLOWED: NO pending canonical v3.**
...
Canonical v2 satisfied the first five closure conditions, but the reviewer’s full result arrived asynchronously after the v2 handoff. Two residual findings were valid:

1. v2 did not hash or directly gate `perf/cycle_model.[ch]`, its focused test, the power focused test, `tests/test_config.c`, or the SRAM-owned `docs/bandwidth-modeling.md`, even though the chapter made boundary claims about those surfaces;
2. the hierarchy probe passed a one-byte object while declaring a 64-byte request, making the unchanged-data check fragile.

Canonical v3 must add 23-hash/62-predicate/85-check coverage, runner-link cycle-model 21/21 and power 20/20, classify the config suite's lack of DRAM assertions and the SRAM document's ownership, verify availability-overwrite semantics, and replace the marker with an all-bytes-checked 64-byte sentinel. V1/v2 remain immutable history.
