...
**Status:** canonical v2 retained; full asynchronous skeptical-review output re-blocked drafting pending a narrow canonical-v3 reseal covering separate cycle/power/config/doc evidence and a 64-byte hierarchy sentinel.
...
17. the source-present/non-archive cycle model's own 21/21 focused suite and distinct row-buffer implementation;
18. the linked power model's 20/20 suite and direct full-config bandwidth consumer;
19. absence of DRAM assertions in `tests/test_config.c`, even though parser fields exist;
20. `docs/bandwidth-modeling.md` as SRAM-owned contrast evidence rather than a DRAM authority;
21. three fixed-time same-channel accesses showing availability overwrite (`1000, 1050, 1050`) rather than queued growth;
22. an object-size-matched 64-byte hierarchy sentinel with every byte verified unchanged.
