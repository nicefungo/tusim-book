...
Canonical v1 and v2 remain retained as provisional history. V1 review rejected incomplete configuration/lifecycle gates and optimizer-removable validation. V2 repaired those items, but the complete source census then exposed additional metric-range/unit, cycle-DMA, bank-statistic, reset, cached-total, and VCD-ordering limitations. This live report describes the expanded v3 candidate; only a successful v3 seal and exact post-v3 review may authorize drafting.

## Source gate

`experiments/ch17_source_audit.py` pins 31 source/config/test/doc hashes and 95 structural, caller, configuration, linkage, aggregate-membership, and documentation predicates: 126 checks total. It fails closed on pin/hash/predicate drift and inventories non-test callers.

Expected authority line:

```text
CH17_SOURCE_AUDIT PASS pin=e918c80b6fce833cd1fcae97730fa841c2176f25 hashes=31 predicates=95 checks=126
```
