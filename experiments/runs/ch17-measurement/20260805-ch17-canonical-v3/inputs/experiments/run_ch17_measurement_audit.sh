...
RUN_ID=${CH17_RUN_ID:-20260805-ch17-canonical-v3}
...
body(){
 ...
 grep -F "CH17_SOURCE_AUDIT PASS pin=$PIN hashes=31 predicates=95 checks=126" "$RUN_DIR/source-audit.log"
...
PY
 ...
 cp "$BOOK_ROOT/experiments/ch17_predraft_validate.py" "$WORK/ch17-predraft-validator-mutated.py"
 printf '\nassert(False)\n' >> "$WORK/ch17-predraft-validator-mutated.py"
 set +e
 python3 "$WORK/ch17-predraft-validator-mutated.py" > "$RUN_DIR/validator-mutation-normal.log" 2>&1; vmn_rc=$?
 python3 -O "$WORK/ch17-predraft-validator-mutated.py" > "$RUN_DIR/validator-mutation-optimized.log" 2>&1; vmo_rc=$?
 set -e
