...
- **C17.11 (qualified):** Reset reconstructs counters at the preserved clock frequency and preserves the entire prior power substructure, including accumulated energy as well as parameters, but `tu_perf_init()` unconditionally re-enables the outer counter. A disabled counter therefore becomes enabled after reset. The comment “keep energy parameters” is narrower than behavior; reset is neither a full energy reset nor enable-state preserving.
- **C17.12 (qualified):** Each derived metric has its own denominator. DMA/DRAM bandwidth divide bytes by seconds derived from caller-authored `total_cycles` and MHz; TOPS divides MAC count by the same derived seconds; MAC efficiency divides MAC count by compiled `TU_PE_ROWS*TU_PE_COLS*total_cycles`, not by frequency or runtime core geometry; energy/MAC divides the sum of six component-energy fields by MAC count; and average power divides that component-energy sum by derived seconds. MAC efficiency is unclamped and can exceed one; `spad_hit_rate = 1-conflicts/accesses` can be negative. Embedded `power_mw` then applies an extra factor of 1,000: 1 pJ over 1 ns reports 1,000 mW rather than 1 mW.
