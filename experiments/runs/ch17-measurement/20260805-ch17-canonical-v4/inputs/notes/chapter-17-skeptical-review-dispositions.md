...
**Verdict:** canonical v1–v3 received **REVISE**. V4 incorporates R15–R21, including the exact-v3 reset, denominator, and scope findings. Drafting remains blocked until canonical v4 passes and an exact post-v4 review accepts it.
...
17. **R17 — optimizer-removable predraft validation: NEW FINDING, SUPERSEDED BY R20.** V2 removed direct assertion-based gates, but its token scan and synthetic mutation did not prove rejection of all Python assertion syntax. R20 records the complete repair.
...
19. **R19 — omitted source-census limitations: NEW FINDING, RESOLVED FOR V3.** The ledger and structural predicates cover merge retention of destination outer configuration; unclamped perf ratios; the embedded average-power unit error and unproduced total; nonmonotonic logging export; bank conflict/utilization semantics; reversed cycle-DMA DRAM timing direction; cycle-reset frequency/perf behavior; standalone reset area loss; and the const-cast energy getter. The probe directly discriminates merge, ratios/unit behavior, bank semantics, and DMA direction; the remaining source-bounded findings are not claimed as executable probe cases. V1/v2 remain provisional evidence rather than drafting authority.
20. **R20 — exact v2 re-review: MAJOR, RESOLVED FOR V3.** The reviewer additionally found that additive-time coverage omitted internal DMA and descriptor-helper composition, and that the validator mutation was synthetic and token-based. V3 enumerates both timing paths. Its runner appends a real `assert(False)` to a disposable validator copy; AST-based detection rejects that exact mutated source under normal and optimized Python.
21. **R21 — exact v3 re-review: MAJOR, RESOLVED FOR V4.** V3 omitted that perf reset re-enables a disabled counter, misstated unlike metric denominators as one common cycles/frequency denominator, and overstated probe scope. V4 adds a disabled/777-MHz reset sentinel and structural gate; splits C17.12 into exact bandwidth, TOPS, efficiency, energy/MAC, and power denominators; and narrows R19 to distinguish executable discriminators from source-only predicates.

## Hand recomputation record

- Perf additive time: `(10+2)+(5+1)=18`; duplicate explicit tick `18+6=24`. Reset preserves 777 MHz and 99 pJ accumulated MAC energy, clears total cycles, and re-enables the disabled outer counter.
