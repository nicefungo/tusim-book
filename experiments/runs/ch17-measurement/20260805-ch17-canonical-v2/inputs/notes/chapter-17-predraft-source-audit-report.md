...
Canonical v1 remains retained as provisional history. Independent review accepted its arithmetic and provenance but rejected its incomplete configuration/lifecycle gates and optimizer-removable validator. This live report describes the repaired v2 candidate; only a successful v2 seal and post-v2 review may authorize drafting.

## Source gate

`experiments/ch17_source_audit.py` pins 31 source/config/test/doc hashes and 80 structural, caller, configuration, linkage, aggregate-membership, and documentation predicates: 111 checks total. It fails closed on pin/hash/predicate drift and inventories non-test callers.

Expected authority line:

```text
CH17_SOURCE_AUDIT PASS pin=e918c80b6fce833cd1fcae97730fa841c2176f25 hashes=31 predicates=80 checks=111
```
...
- field-complete diff/merge omission sentinels for WS/OS cycles, GBuf conflicts, DRAM row hits/misses, all bandwidth-utilization fields, wall time, all energy parameters and power-enable state, plus cached total energy for merge;
