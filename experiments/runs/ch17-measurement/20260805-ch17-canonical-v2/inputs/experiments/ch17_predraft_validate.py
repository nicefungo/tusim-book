...
assert_token=''.join(['ass','ert',' '])
if os.environ.get('CH17_VALIDATOR_MUTATION')=='assert-source':
 require(assert_token not in assert_token+'False','optimizer-removable assertion in validator')
 raise SystemExit('CH17_PREDRAFT_VALIDATION FAIL: validator mutation unexpectedly survived')
run_id=os.environ.get('CH17_RUN_ID','20260805-ch17-canonical-v2')
...
'PERF_TIME_MAP op=5 idle=2 spad=3 dram=4 timed=14 after_no_time=14',
'PERF_DIFF_OMISSIONS ws=0 os=0 gbuf=0 hits=0 misses=0 bw=0.0/0.0/0.0 wall=0 params=0/0/0/0/0/0 enabled=0',
'PERF_MERGE_OMISSIONS ws=0 os=0 gbuf=0 hits=0 misses=0 bw=0.0/0.0/0.0 wall=0 total=0.0 params=1.00/0.50/0.50/20.0/0.050/0.001 enabled=0',
...
require(assert_token not in validator_source,'optimizer-removable assertion in validator')
