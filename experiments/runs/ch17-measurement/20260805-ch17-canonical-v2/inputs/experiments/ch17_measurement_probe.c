...
int main(void) {
    ...
    printf("PERF_TIME_MAP op=5 idle=2 spad=3 dram=4 timed=%lu after_no_time=%lu\n",
      (unsigned long)timed,(unsigned long)tm.total_cycles);
    CHECK(timed==14 && tm.total_cycles==14);
    ...
    tu_perf_counters_t m; tu_perf_init(&m,1000.0); m.power.power_modeling_enabled=false; tu_perf_merge(&m,&r);
    ...
    printf("PERF_MERGE_OMISSIONS ws=%lu os=%lu gbuf=%lu hits=%lu misses=%lu bw=%.1f/%.1f/%.1f wall=%lu total=%.1f params=%.2f/%.2f/%.2f/%.1f/%.3f/%.3f enabled=%d\n",
      ...
      m.power.pj_leakage_per_cycle,m.power.power_modeling_enabled);
    ...
    CHECK(!m.power.power_modeling_enabled);
