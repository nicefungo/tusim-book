...
RUN_ID=${CH17_RUN_ID:-20260805-ch17-canonical-v1}
...
body(){
 ...
 grep -F "CH17_SOURCE_AUDIT PASS pin=$PIN hashes=31 predicates=65 checks=96" "$RUN_DIR/source-audit.log"
...
( cd "$RUN_DIR"; find inputs -type f -print0 | sort -z | xargs -0 sha256sum; sha256sum input-hashes.txt input_commit source_pin tusim-ignored-before.sha256 tusim-ignored-after.sha256 source-audit.log source-audit-mutation.log source-audit-restored.log build.log archive-members.log *-readelf.log test-perf.log test-trace.log test-logging.log test-power.log test-cycle.log test-perf-mutation.log test-bench.log probe.log transcript.log ) > "$RUN_DIR/sha256-retained.txt"
