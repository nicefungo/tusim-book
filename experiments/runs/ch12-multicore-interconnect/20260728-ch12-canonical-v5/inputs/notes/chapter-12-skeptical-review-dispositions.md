...
- **Current canonical target:** `experiments/runs/ch12-multicore-interconnect/20260728-ch12-canonical-v5`
...
**Disposition:** **Resolved.** Canonical-v3 seals the counterexample in the probe, expected transcript, report, and manifest. The chapter and affected claims use neutral heuristic terminology.
...
**Disposition:** **Resolved.** Functional claims require bounded, in-range inputs and successful return. Canonical-v3 hashes and audits SRAM behavior; broadcast is explicitly sequential and non-transactional. Unsafe boundary executions remain excluded.
...
**Disposition:** **Resolved.** Canonical-v3 adds exact hashes and scoped predicates and checks complete shipped JSON/YAML multicore/ICC values.
...
**Disposition:** **Resolved.** Canonical-v3 labels exact C-call inventories and splits transfer, send, broadcast, all-reduce, barrier, and SPMD sets.
...
**Disposition:** **Qualified.** Retention and direct consumer behavior remain separate claims because canonical-v5 does not add one-field-at-a-time parse→convert→construct→effect checks.
...
**Disposition:** **Resolved.** Canonical-v3 checks the contradictions explicitly; they remain documentation drift, never confirming evidence.
...
**Disposition:** **Resolved.** The probe retained by canonical-v5 compares descriptor fields individually, initializes config state, and stops dependent checks after parse failure.

### R32 — Extreme final traffic-score addition can overflow

**Finding:** the declared 32-bit message count and size widths bound any one directed-link accumulator below `UINT64_MAX`; the unchecked overflow risk is the later `bottleneck_link_cycles + max_route_cycles` addition.

**Disposition:** **Reclassified and qualified.** Canonical-v4 must record the type-width proof and missing final-addition guard. Unsafe extreme cases remain excluded behind a trusted-caller bound.

## Gate decision

**Current verdict:** **BLOCK pending canonical-v5.** Canonical-v3 corrected the heuristic classification but later manuscript/reproducibility review found arithmetic-qualification and bundle-closure defects. Canonical-v4 then failed its retained validator before outer sealing. Canonical-v2 through v4 remain immutable historical evidence; none is the closure seal.
