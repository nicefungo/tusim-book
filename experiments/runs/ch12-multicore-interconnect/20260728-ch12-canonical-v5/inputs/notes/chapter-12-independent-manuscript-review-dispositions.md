...
- **Replacement canonical target:** `experiments/runs/ch12-multicore-interconnect/20260728-ch12-canonical-v5/`
- **Current closure status:** blocked pending canonical-v5 and clean re-review
...
**Disposition:** accepted. The closure validator must enforce exact inner/outer member sets, transcript digest consistency, direct config-log qualification, clean Git state, and byte equality with committed blobs. These checks become mandatory after canonical-v5 is committed.

### R4 — Live documents retained stale canonical-v2/v3 anchors

**Disposition:** accepted. Current status surfaces now target canonical-v5; references to earlier runs remain only where explicitly labeled immutable historical evidence, including canonical-v4's retained-validator failure before outer sealing.

## Gate decision

**BLOCK pending canonical-v5 and clean re-review.** No reviewer PASS is carried forward across the corrected evidence snapshot.