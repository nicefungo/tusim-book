...
RUN_REL = Path("experiments/runs/ch12-multicore-interconnect/20260728-ch12-canonical-v3")
RUN = ROOT / RUN_REL
...
assert "C12.1" in ledger and "C12.46" in ledger
assert "C12.47" not in ledger
for phrase in (
...
for line in (
    ...
    "SEND blocking=%d",
    "TRAFFIC same=%",
    "HEURISTIC_COUNTEREXAMPLE isolated=%",
    "ROUTES patternA_XY=%",
...
for phrase in (
    ...
    "link-service-no-overflow-guard",
...
for gate in (
    f"CH12_SOURCE_AUDIT PASS pin={PIN} hashes=28 predicates=149 checks=177",
    "SOURCE_AUDIT_MUTATION PASS",
