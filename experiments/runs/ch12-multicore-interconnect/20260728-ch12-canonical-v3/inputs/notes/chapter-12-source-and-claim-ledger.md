...
- **Draft gate:** reopened after independent reviewers disproved the lower-bound classification; canonical-v3 pending
...
| C12.43 | The combined shared-link equation can exceed a feasible schedule when bottleneck service and maximum route latency come from disjoint flows. | source; adversarial 4×4 XY counterexample | blocked | canonical-v3 must require `isolated=94 bottleneck=128 estimated=158` and classify it as a disproved bound label |
| C12.44 | Directed-link service accumulation uses unchecked `uint64_t` addition. | static source audit | qualified | extreme message-count/size inputs require trusted-caller bounds; do not execute overflow cases |
...
| C12.46 | The config generator consumes YAML multicore/ICC fields into compile-time header definitions, but generated enable/count/topology macros do not instantiate a runtime cluster. | generator hash/source audit | blocked | seal exact generator/config predicates in canonical-v3 |
...
Tusim reaches different rungs for point-to-point send, traffic analysis, broadcast, all-reduce, barrier, and SPMD. C12.43 and C12.46 remain blocked pending canonical-v3; canonical-v2 is retained as superseded historical evidence.
