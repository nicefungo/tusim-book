...
- **Chapter:** Multi-Core Clusters and Interconnect Lower Bounds
- **Tusim pin:** `e918c80b6fce833cd1fcae97730fa841c2176f25`
- **Canonical runner:** `experiments/run_ch12_multicore_interconnect_audit.sh`
- **Canonical retained run:** `experiments/runs/ch12-multicore-interconnect/20260728-ch12-canonical-v2/`
...
Given core-local state, a point-to-point or collective operation, and a traffic matrix, what does pinned Tusim execute functionally, which configuration reaches that behavior, and which cycle quantity is an isolated estimate, a simultaneous shared-link lower bound, an additive counter, or absent?
...
- a precise deterministic-lower-bound fidelity label.
...
An initial sealed execution at `20260728-ch12-canonical` completed the source, build, mutation, test, sweep, probe, provenance, and manifest gates, but exposed a false-negative in the pre-draft validator: it searched for a rendered CONFIG value inside split C `printf` source. That attempt remains retained as debugging evidence but is superseded. The corrected validator checks the format string and is frozen into the `20260728-ch12-canonical-v2` input commit before re-execution.
...
- whole-tree C caller sets for functional cluster methods, construction, and traffic estimation;
- linked traffic sweeps versus standalone duplicated-formula sweeps.

At the frozen audit-input snapshot this is 116 structural/reachability predicates, for 141 source checks including hashes. The canonical transcript must contain:

```text
CH12_SOURCE_AUDIT PASS pin=e918c80b6fce833cd1fcae97730fa841c2176f25 hashes=25 predicates=116 checks=141
```
...
### Traffic estimation is a simultaneous lower bound
...
max(max isolated transfer,
    maximum directed-link serialization load + maximum route latency)
```

It has no injection times, arbitration, finite buffers, virtual channels, credit flow, head-of-line blocking, or queue schedule.

### Route order is traffic-specific

For a 4x4 mesh, nine 1 KiB messages from the non-root top row to the non-root left column concentrate deterministic XY routes; transposing the pattern concentrates YX routes. Expected custom-probe observation:

```text
ROUTES patternA_XY=606 patternA_YX=222 patternB_XY=222 patternB_YX=606
```

The linked 4 KiB sweep gives the same ranking reversal at a different payload, while symmetric all-to-all gives equal XY/YX bounds. This supports traffic-regime comparison, not a universal route-order winner.
...
Barrier increments `stats.total_barriers` and adds exactly twice router latency to each core. It has no arrival, wait, or topology-dependent rendezvous state; the separate `barrier_counter` field remains zero.
...
The contention and route-order sweeps call `tu_cluster_estimate_traffic_cycles()` and are compiled against the static archive. Their tables remain analytical lower-bound observations.
...
| traffic matrix | deterministic simultaneous lower bound |
...
| five ICC timing/routing fields | parsed, converted, retained, consumed, effect-tested |
...
The retained manifest uses only run-relative paths and includes the complete transcript. No source archive, extracted source tree, object file, binary, or core file is retained as evidence.

## Verdict

The audit supports a chapter about **functional cluster helpers plus deterministic interconnect estimates and lower bounds**. It rejects a chapter claiming concurrent cores, packet transport, complete collective timing, cycle-accurate NoC behavior, or calibrated multicore speedup.
