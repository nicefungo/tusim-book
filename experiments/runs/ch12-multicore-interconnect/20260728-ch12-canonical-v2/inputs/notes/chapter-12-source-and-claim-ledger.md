# Chapter 12 Source and Claim Ledger — Multi-Core Clusters and Interconnect Lower Bounds

- **Pinned source:** `e918c80b6fce833cd1fcae97730fa841c2176f25`
- **Canonical run target:** `experiments/runs/ch12-multicore-interconnect/20260728-ch12-canonical-v2/`
- **Status vocabulary:** `verified`, `qualified`, `rejected`, `blocked`
- **Draft gate:** blocked pending fail-closed audit, skeptical review, evidence sealing, and pre-draft validation
...
| C12.2 | Core operation wrappers swap the selected state through process-global `g_tu`, execute a legacy direct API, and copy state back. | `tu_core.c`; call inventory | verified | serialized facade; no thread-safety or simultaneous execution claim |
...
| C12.4 | W/A/O SRAM allocations are distinct across focused-created cores and retain different test values. | focused multicore test; custom probe | verified | observed storage isolation does not prove all singleton subsystems are isolated |
| C12.5 | Cluster construction accepts 1–256 cores and explicit NONE/RING/MESH topology; a mesh requires nonzero rows and derives columns by ceiling division. | cluster source/probe | verified | topology and count are API arguments, not auto-created from full config |
...
| C12.14 | The traffic estimator interprets all input messages as simultaneous, computes maximum isolated latency, and optionally adds a shared directed-link serialization lower bound. | source; linked sweep/probe | verified | deterministic lower bound, not queue schedule |
...
| C12.16 | Ideal-parallel returns the maximum isolated message estimate; shared-link returns `max(isolated, bottleneck serialization + maximum route latency)`. | source; exact probe | verified | lower bound omits injection, arbitration, buffers, VCs, and HOL blocking |
| C12.17 | Two same-link 1 KiB cut-through messages at 16 B/cycle and 5 cycles/router produce 133 cycles; disjoint links and ideal-parallel produce 69. | focused test/canonical probe | blocked | promote after canonical execution |
| C12.18 | On the selected asymmetric 4x4 traffic, XY and YX produce different bottlenecks and exact lower bounds; the transposed pattern reverses the winner. | route sweep/custom probe | blocked | traffic-specific, not universal route recommendation |
| C12.19 | Functional broadcast copies source bytes to every other core through N-1 calls to send. | source; focused/probe | verified | no multicast tree or simultaneous fan-out |
| C12.20 | Functional FP32 all-reduce reads every core on the host, sums in core-index order, and writes the result to every core. | source; focused/probe | verified | host reduction; no routed collective algorithm |
| C12.21 | All-reduce increments only N-1 message and gather-byte counts and adds no ICC cycles or per-core estimated cycles. | source; counter-delta probe | blocked | promote after canonical execution; reported traffic is incomplete for gather+broadcast |
| C12.22 | Barrier increments `stats.total_barriers` and adds exactly `2 * hop_latency` to every core without recording arrival or waiting state; the separately declared `barrier_counter` is not updated. | source; cross-topology/counter probe | blocked | functional no-op plus estimate, not rendezvous simulation |
| C12.23 | SPMD execution is implementation-only in the audited whole-tree C caller inventory; its implementation loops over cores sequentially and calls the text interpreter without host threads or a common start event. | source and exact caller audit | verified | “SPMD API loop,” not concurrent launch; no executable API probe at this pin |
| C12.24 | The focused multicore source invokes 16 named test functions and exits nonzero on recorded failure. Its SPMD-named case directly calls per-core MMA and does not call `tu_cluster_spmd_execute()`. | test source; forced-failure mutation in disposable archive; canonical run | blocked | 16/16 is a regression snapshot for covered assertions, not SPMD API coverage or multicore certification |
...
| C12.28 | Full config parses and converts switching, contention, mesh routing, link bytes/cycle, and router latency; cluster creation retains and consumes those runtime fields. | config/cluster source; A/B probe; config test | blocked | promote after canonical effect probe |
...
| C12.32 | Current linked traffic evidence can reverse ring/mesh or XY/YX ranking by traffic shape. | canonical linked sweeps | blocked | no universal topology winner; report exact workload and assumptions |
...
| C12.34 | Cluster timing, traffic, barrier, and core `estimated_cycles` values are not externally calibrated against RTL, FPGA, or silicon in the audited evidence. | source/audit/status review | verified | label estimated or deterministic lower bound |
...
| C12.40 | `barrier_counter` remains zero across a barrier while `stats.total_barriers` increments. | source and canonical counter probe | blocked | distinguish the unused lifecycle field from the statistics count |
| C12.41 | The current multicore guide's claims that cores never share mutable state, SPMD programs start simultaneously, and barriers synchronize are stronger than the pinned implementation supports. | guide-versus-source/caller audit | rejected | describe a serialized global-state facade, implementation-only serial SPMD loop, and no-wait barrier estimate |
...
| `isolated_cycles` | maximum isolated estimate across one simultaneous message array | ideal-parallel lower bound for that array |
| `bottleneck_link_cycles` | sum of serialization service on busiest directed route edge | link-load term, not elapsed link trace |
| `estimated_cycles` in traffic stats | max of isolated and shared-link bound | deterministic traffic-matrix lower bound |
...
| contention mode | yes | yes | yes | traffic-matrix bound | no |
...
- **Analytical model / deterministic lower bound:** isolated switching equations, shared-link traffic estimate, barrier constant, and historical sweeps.
...
-> simultaneous traffic lower bound -> queued/arbitrated network -> calibrated behavior
```

Tusim reaches different rungs for point-to-point send, traffic analysis, broadcast, all-reduce, barrier, and SPMD. Drafting remains blocked until blocked claims are promoted or qualified by canonical execution and skeptical review.
