# Chapter 12 Framing and Evidence Plan — Multi-Core Clusters and Interconnect Lower Bounds
...
2. Does the boundary have one coherent executable and estimate domain?
...
|---:|---|---|---|---|
| 1 | **Multi-Core Clusters and Interconnect Lower Bounds** | decide whether a placement/traffic pattern is functionally represented and which topology/switching/contention/routing estimate is safe; linked core/cluster code, 16-case focused test, config propagation, and discriminating traffic probes share one cluster boundary | easy to mistake immediate host copies and lower-bound equations for a concurrent NoC | **selected** |
...
- the interconnect candidate must separate cluster ownership from topology analysis, yet uniquely combines byte-observable public effects, five configuration fields with exact runtime consumers, linked traffic-matrix lower bounds, and route/contention probes.
...
3. a simultaneous traffic-matrix lower bound is not a finite-router schedule;
...
1. `tu_core_t` as a stored `tu_state_t` snapshot operated through serialized swap-in/swap-out wrappers.
...
| linked traffic sweeps | `tests/test_interconnect_contention_sweep.c`, `tests/test_interconnect_routing_sweep.c` | how do traffic shape and route order affect the implemented lower bound? |
...
The five timing/routing fields are expected to reach the discriminating-effect rung. `multicore_enabled`, `num_cores`, and `interconnect_mode` are expected to stop before runtime conversion and cluster instantiation. No selected field is expected to reach external calibration.
...
- Requested runtime switching/link/router/routing/contention values retained in a real cluster and reflected in exact estimates.
...
- Same-link versus disjoint-link traffic; ideal-parallel versus shared-link bounds.
- XY/YX reversal under transposed asymmetric traffic and equality for symmetric all-to-all.
- Broadcast byte effects and sum-of-sequential-send stats.
- All-reduce byte correctness with zero routed-cycle increment.
- Barrier's fixed `2 * hop_latency` increment independent of topology/core count.
- Configuration parse/conversion split: five ICC timing/routing fields propagate; cluster-enable/count/topology do not.
...
- Are isolated transfer, traffic lower bound, destination-core accumulation, and elapsed time kept distinct?
...
- Is “lower bound” repeated wherever shared-link results appear?
