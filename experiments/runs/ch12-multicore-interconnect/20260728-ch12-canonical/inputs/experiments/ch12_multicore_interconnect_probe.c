...
static void probe_send(void) {
    ...
    CHECK(memcmp(&msg, &original, sizeof(msg)) == 0,
...
static void probe_traffic(void) {
    ...
    CHECK(tu_cluster_estimate_traffic_cycles(&c, same, 2, &s) == 0 &&
          ...
          "same directed link exposes shared serialization bound");
    CHECK(tu_cluster_estimate_traffic_cycles(&c, disjoint, 2, &d) == 0 &&
          d.estimated_cycles == 69,
          "disjoint directed links retain isolated lower bound");
...
static uint32_t asymmetric(tu_icc_message_t *m, bool transpose) {
    uint32_t count=0;
    if (!transpose) {
...
static void probe_routes(void) {
    ...
    n=asymmetric(m,true);
    c.mesh_routing_mode=TU_ICC_MESH_ROUTE_XY; CHECK(tu_cluster_estimate_traffic_cycles(&c,m,n,&bxy)==0,"XY transpose estimated");
    c.mesh_routing_mode=TU_ICC_MESH_ROUTE_YX; CHECK(tu_cluster_estimate_traffic_cycles(&c,m,n,&byx)==0,"YX transpose estimated");
    CHECK(axy.estimated_cycles==606 && ayx.estimated_cycles==222 &&
          bxy.estimated_cycles==222 && byx.estimated_cycles==606,
          "transpose reverses deterministic route-order winner");
