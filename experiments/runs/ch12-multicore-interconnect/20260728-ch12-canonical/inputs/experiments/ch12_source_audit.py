...
def main() -> int:
    ...
    must(core, "static void core_swap_in", "serialized-swap-in-helper")
    must(core, "static void core_swap_out", "serialized-swap-out-helper")
    ...
    must(cluster_h, "deterministic-routing\n * lower bound", "header-lower-bound-label")
    must(cluster, "uint64_t shared_bound = stats->bottleneck_link_cycles + max_route_cycles;", "shared-link-lower-bound-equation")
    ...
    for field in ["multicore_enabled", "num_cores", "interconnect_mode"]:
        must(full_cfg, field, f"full-config-declares-parses-{field}")
    ...
    must(shipped_json, '"enabled": false', "json-multicore-disabled")
    must(shipped_json, '"interconnect": "none"', "json-topology-none")
    must(shipped_yaml, 'switching: "legacy_hop_only"', "yaml-switching-legacy")
    ...
    inventories = [
        (r"tu_cluster_estimate_traffic_cycles\(", {
         ...
         "whole-tree-traffic-estimator-callers"),
        (r"tu_cluster_(?:send|broadcast|allreduce_sum_f32|barrier)\(", {
            "tu_cmodel/tu_cluster.c", "tests/test_multicore.c"},
         "whole-tree-functional-communication-callers"),
        ...
        (r"tu_cluster_create\(", {"tu_cmodel/tu_cluster.c", "tests/test_multicore.c"},
         "whole-tree-cluster-constructor-callers"),
    ]
