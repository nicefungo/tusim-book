...
- **Canonical retained run:** `experiments/runs/ch12-multicore-interconnect/20260728-ch12-canonical-v4/`
...
An initial sealed execution at `20260728-ch12-canonical` completed the source, build, mutation, test, sweep, probe, provenance, and manifest gates, but exposed a false-negative in the pre-draft validator. Canonical-v2 fixed that validator and sealed the expected outputs. A later independent skeptical review disproved the manuscript's classification of the combined global-max equation as a makespan lower bound and identified omitted generator/SRAM evidence. Canonical-v3 corrected those findings, but independent manuscript/reproducibility review then found arithmetic qualifications, stale anchors, and bundle-closure defects. All earlier runs remain immutable historical evidence; canonical-v4 is the pending replacement seal.
...
At the canonical-v4 audit-input snapshot this is 155 structural/reachability predicates, for 183 source checks including hashes. The canonical transcript must contain:
...
**Pending canonical-v4.** The corrected inputs support a chapter about **adjacent functional cluster helpers, isolated equations, and deterministic route-load heuristic scores**, but closure remains blocked until the runner retains a passing pre-draft validation and Git-pinned outer bundle manifest. The chapter rejects one integrated NoC model, concurrent cores, packet transport, a proved traffic makespan bound, complete collective timing, cycle-accurate behavior, or calibrated multicore speedup.
