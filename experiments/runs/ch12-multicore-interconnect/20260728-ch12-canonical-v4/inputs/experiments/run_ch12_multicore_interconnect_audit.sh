...
INPUTS=(
    ...
    notes/chapter-12-independent-manuscript-review-dispositions.md
)
BUNDLED_INPUTS=()
...
body() {
    ...
    grep -Fq "CH12_SOURCE_AUDIT PASS pin=$PIN hashes=28 predicates=155 checks=183" \
...
CH12_RUN_ID="$RUN_ID" python3 "$BOOK_ROOT/experiments/ch12_predraft_validate.py" \
    | tee "$RUN_DIR/predraft-validation.log"
grep -Fq "CH12_PREDRAFT_VALIDATION PASS" "$RUN_DIR/predraft-validation.log"
