...
def execute(book,tusim,runid):
 ...
 with tempfile.TemporaryDirectory(prefix='ch22-controls-') as td:
  ...
  src=(book/'experiments/ch22_predraft_validate.py').read_text();needle='from __future__ import annotations\n';mutated=td/'validator-with-real-assert.py';mutated.write_text(src.replace(needle,needle+"assert False, 'real assertion mutation control'\n",1))
  an=py(mutated,args[:-1]+[td/'assert-normal.json'],book,ok=(1,));ao=py(mutated,argso[:-1]+[td/'assert-opt.json'],book,opt=True,ok=(1,))
  (rd/'assertion-control.json').write_text(json.dumps({'normal_returncode':an.returncode,'optimized_returncode':ao.returncode,'passed':an.returncode!=0 and ao.returncode!=0},indent=2,sort_keys=True)+'\n')
...
def verify(book,tusim,runid):
 ...
 outer=book/'notes/chapter-22-postreview-seal.json'
 if outer.is_file():
  o=json.loads(outer.read_text());bc=o['bundle_commit'];tree=git(book,'rev-parse',f"{bc}:experiments/runs/ch22-predraft/{runid}")
 ...
 print(json.dumps({'verify':'PASS','run':str(rd.relative_to(book)),'payload_members':len(inner['payload_members']),'outer_seal':outer.is_file()},sort_keys=True))
