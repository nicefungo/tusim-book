...
CH11_RUN_ID=20260728-ch11-canonical \
  bash experiments/run_ch11_instruction_contract_audit.sh
```

The runner fails provenance before durable output, hashes all execution inputs, exports the exact detached pin into a disposable tree, builds only a static archive in that tree, runs a fail-closed source/reachability audit, compiles four pinned focused harnesses and one adversarial probe explicitly against that archive, rejects a shared `libtucmodel` dependency, captures stderr with each family log, checks source and book state after execution, hashes the closed transcript in a second phase, and verifies the retained manifest. It does not invoke the pinned Makefile's `clean` target because that target removes process-global `/tmp` paths outside its build tree.
