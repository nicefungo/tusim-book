    ...
    if len(sys.argv)==4 and not failures:
        ...
        for p in reports:
            ...
            report_rows.append({
                ...
                "question":bool(re.search(r"(?im)^\\*\\*Question:\\*\\*|^##?\\s+Question",text)),
                "hypothesis":bool(re.search(r"(?im)^\\*\\*Hypothesis:\\*\\*|^##?\\s+Hypothesis",text)),
                "explicit_harness":bool(re.search(r"tests/test_[A-Za-z0-9_]+\\.c|scripts/sweep_[A-Za-z0-9_./-]+\\.py",text)),
                "repro_command":bool(re.search(r"make\\s+test-[A-Za-z0-9-]+|python3?\\s+scripts/",text)),
            })
        payload={"pin":PIN,"source_target_pairs":[{"source":s,"target":t} for s,t in sorted(PAIRS)],"no_rule":["tests/test_int8_sweep.c"],"aggregate_exploration_targets":[],"reports":report_rows,"producer_classes":{"executable_or_linked_inventory":19,"local_formula_inventory":4}}
