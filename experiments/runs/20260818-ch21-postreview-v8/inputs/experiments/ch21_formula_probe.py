...
def dataflow_linked(M:int=128,N:int=128,K:int=256,tile_m:int=16,tile_n:int=16,tile_k:int=16,pd:int=2):
    """Recompute dispatcher+plugin cycles from the pinned per-tile contracts."""
    mt=math.ceil(M/tile_m); nt=math.ceil(N/tile_n); kt=math.ceil(K/tile_k)
    ws=os=rs=0
    for mi in range(mt):
      for ni in range(nt):
        for ki in range(kt):
          kc=min(tile_k,K-ki*tile_k)
          ws += pd*tile_n + kc + pd*tile_m
          os += kc + math.ceil(kc/4)
          rs += (pd-1)*tile_n+1 + kc + (pd-1)*tile_m
    return dict(M=M,N=N,K=K,tile_m=tile_m,tile_n=tile_n,tile_k=tile_k,ws=ws,os=os,rs=rs,producer="linked_plugin_source_equations")
...
def main() -> int:
    ...
    sweep_df=dataflow_sweep(); linked_df=dataflow_linked()
