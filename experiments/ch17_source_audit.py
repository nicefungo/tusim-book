...
# Perf semantics
for needle in ['tu_perf_dma_record_read','tu_perf_dma_record_write','tu_perf_dma_record_internal','tu_perf_compute_record_mma','tu_perf_compute_record_op','tu_perf_compute_record_idle']:
...
desc_helper=section(perf,'void tu_perf_from_dma_descriptor','tu_perf_metrics_t tu_perf_compute_metrics')
pred('descriptor-helper-composite-time','tu_perf_dma_record_read' in desc_helper and 'tu_perf_mem_record_spad_access' in desc_helper)
...
pred('merge-omits-outer-config','dst->clock_freq_mhz' not in perf and 'dst->enabled' not in perf)
pred('reset-preserves-whole-power','tu_power_counters_t saved_power = c->power' in perf and 'c->power = saved_power' in perf)
pred('reset-reenables-counter','tu_perf_init(c, freq)' in section(perf,'void tu_perf_reset','void tu_perf_set_enabled') and 'c->enabled          = true' in section(perf,'void tu_perf_init','void tu_perf_reset'))
pred('metrics-use-compiled-geometry','TU_PE_ROWS * TU_PE_COLS' in section(perf,'tu_perf_metrics_t tu_perf_compute_metrics','/* ================================================================\n * Reporting'))
pred('metrics-ratios-unclamped','(double)c->compute.total_macs / peak_macs' in perf and '1.0f - (float)c->memory.mem_spad_bank_conflicts' in perf)
pred('metrics-power-extra-thousand','/ 1e9 / seconds * 1e3' in perf)
pred('embedded-energy-total-unproduced','c->power.energy_total_pj' not in section(perf,'void tu_perf_power_config','tu_perf_snapshot_t tu_perf_snapshot'))
...
pred('logging-vcd-strict-increase','if (evt->cycle > last_cycle)' in log)
pred('logging-vcd-final-can-regress','fprintf(output, "#%lu\\n", (unsigned long)g_trace_cycle)' in log)
# Cycle model
...
pred('cycle-bank-conflict-is-post-consumption','word_count > 0 && bank->words_available < bm->max_accesses_per_cycle' in cyc)
pred('cycle-bank-utilization-no-time-window','(double)(r + w) / (double)total_cap' in cyc)
pred('cycle-dram-direction-reversed','tu_dram_access(cm->dram_channel, dram_addr,\n                                                is_read, bytes' in cyc)
reset=section(cyc,'void tu_cycle_model_reset','uint64_t tu_cycle_model_execute_tile')
pred('cycle-reset-forces-one-ghz','double mhz = type == TU_DRAM_IDEAL ? 1000.0 : 1000.0' in reset)
pred('cycle-reset-leaves-perf','tu_perf_reset' not in reset)
...
power_reset=section(power,'void tu_power_model_reset','/* ================================================================\n * Energy Recording')
pred('power-reset-loses-area','tu_power_model_init(pm, node, freq)' in power_reset and 'estimated_area_mm2' not in power_reset)
pred('power-const-getter-mutates','tu_power_model_t *mut = (tu_power_model_t *)pm' in section(power,'double tu_power_get_energy_per_mac','tu_power_breakdown_t tu_power_get_breakdown'))
# Caller inventories
