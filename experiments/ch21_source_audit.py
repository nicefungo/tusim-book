...
def main() -> int:
    ...
    os_plugin=(root/"tu_cmodel/compute/dataflow/output_stationary.c").read_text()
    ...
    predicates={
      ...
      "dataflow-linked-ws-cycle-contract":all(x in ws for x in ("return (uint64_t)k_count; /* compute cycles */","return (uint64_t)pd * tile_n;","return (uint64_t)pd * tile_m;")),
      "dataflow-linked-os-cycle-contract":all(x in os_plugin for x in ("return (uint64_t)k_count + (k_count + 3) / 4;","return 0;")),
      "dataflow-linked-rs-cycle-contract":all(x in rs for x in ("return (uint64_t)k_count;","return (uint64_t)(pd - 1) * tile_n + 1;","return (uint64_t)(pd - 1) * tile_m;")),
