...
- **Canonical retained run:** `experiments/runs/ch14-compute-engines/20260804-ch14-canonical-v3/` (seal wording below); retained runs live under `experiments/runs/ch14-compute-engines/`
...
The canonical seal is the run whose retained input hashes matched the book inputs at the sealing commit — `20260804-ch14-canonical-v3`, sealed at book commit `fbb7eef`; the bundled copies under `inputs/` remain the binding evidence and verify against that commit. Post-seal amendments to this report, the framing plan, and the claim ledger (sketched in the skeptical-review dispositions and applied after the seal) are recorded in git history; they do not alter the sealed evidence copies. All earlier attempts, complete or failed, are retained as immutable history (v1 truncated at the attention gate, v2 superseded by the v3 reseal that bundles the review amendments and the verified `[DAO22]` reference), and any reseal supersedes the prior seal only through the same fail-closed gates. The runner may be re-executed with a new `CH14_RUN_ID`; earlier runs are immutable historical evidence. Toolchain recorded for the seal: aarch64 host, `cc` GCC 11.4.0, Make 4.3, Python 3.11.15, coreutils 8.32; `TUSIM_ROOT` is machine-specific and overridable via the environment.
...
- test-membership predicates (`test-softmax` has no Makefile rule — source-present-no-target; a false-positive aggregate membership fails the gate);
...
6. `test-attention` — **qualified, never green**: expects rc=1 with `6/9`–`8/9 tests passed` (observed range across builds; the gate accepts any `1/9`–`8/9` count), at least one `FAIL` line, and never `9/9`. The SRAM access-width defect corrupts FP16 staging with UB-dependent magnitudes (stack garbage varies per build), so the exact failing subset varies (canonical v3: `test_identity` + `test_deterministic_small` + `test_causal` at 6/9; canonical v2 and mid-audit: `test_scale` at 8/9; a rebuild: `test_deterministic_small` + `test_causal` at 7/9). The runner labels `ATTENTIONSUITEQUALIFIED PASS` with the observed summary and the `failing_subset_ub_dependent=yes` flag;
7. `test-softmax` — no Makefile rule (source-present-no-target pattern, like ch13's `test_int8_sweep.c`; excluded from `make test`; compiled directly by the runner); expects `=== Results: 15/15 passed, 0 failed ===`;
